...
class all:
    ...
    def letters():
        for name in Letter.letters:
            yield Letter(name)
    @staticmethod
    def notes(min_octave=4, max_octave=4):
    ...
    def dyads():
        for note in all.notes():
            for name in Interval.intervals:
                yield Dyad(note, name)
    def chords(min_octave=4, max_octave=4, root=None):
        if root is None:
            roots = all.notes()
    ...
    @staticmethod
    def scales(include_greek_modes=False):
...
class Note:
    ...
    * octave (default is 4).
    ...
    def __eq__(self, other): 
        #g0s : might remove scientific_notation usage as it limits my implementation of music theory.
        return self.scientific_notation() == other.scientific_notation() or str(self) == str(other)
        
    def __hash__(self):
        #needed to turn list of notes into set.
...
class Dyad:
    ...
    def __init__(self, root, interval_value ):
        if isinstance(root, str):
            root = Note(root)
        if not isinstance(root, Note):
            raise ValueError('Invalid root: {!r}'.format(root))

        self.interval = Interval(interval_value)
        self.root = root
        self.notes = [root, root + self.interval]

    def __contains__(self, k):
        return k in self.notes

    def __repr__(self):
        return "Dyad({!r}, {!r})".format(self.root, self.interval)

    def __str__(self):
        return "{} {}".format( self.root, self.interval )
...
class Voicing:
    ...
    Currently only supports drop 2 voicings.
    """
    
    def __init__(self, notes ):
        self.drop2 = [notes.pop(-2)] + notes
...
class Chord(Voicing):
    ...
    def __init__(self, root, chord_type='M', inversion = 'root' ):
        if isinstance(root, str):
        ...
        if inversion in self.cardinals and self.cardinals[inversion] < len(self.recipes[chord_type]):
            inversion = self.cardinals[inversion]
        else:
            raise TypeError('{} is an invalid inversion for {}.'.format(inversion,chord_type))
        ...
        self.notes = self.notes[self.inversion:] + [notes + Interval('P8') for notes in self.notes[:self.inversion]]
        Voicing.__init__(self,self.notes[:])
        
    def degree(self, y ):
        return "Chord({!r}, {!r})".format(self.root, self.chord_type)
    ...
    def __str__(self):
        return "{}{}".format(str(self.root), self.chord_type)

    def __eq__(self, other):
        if isinstance(other, Chord):
            return Counter(self.notes) == Counter(other.notes)
        else:
            if builtins.all(isinstance(other[i], Note) for i in range(len(other))):
                return Counter(self.notes) == Counter(other)
